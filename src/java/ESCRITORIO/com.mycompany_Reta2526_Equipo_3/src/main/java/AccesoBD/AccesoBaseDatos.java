...
public class AccesoBaseDatos {
    ...
    private DataTunel tunel; // ← añadir

    private AccesoBaseDatos() {
        tunel = new DataTunel(); // ← añadir
        abrirConexion();
    }

    private void abrirConexion() {
        // 1. Abrir el túnel SSH ANTES de conectar JDBC
        try {
            if (!tunel.estaConectado()) {
                LoggerApp.log("Abriendo túnel SSH...");
                tunel.abrirTunel();
                Thread.sleep(500);
                LoggerApp.log("Túnel SSH abierto correctamente.");
            }
        } catch (Exception ex) {
            LoggerApp.log("❌ Error al abrir el túnel SSH.");
            LoggerApp.log("Mensaje SSH: " + ex.getMessage());
            conn = null;
            return;
        }

        // 2. Cargar properties (tu código actual, sin cambios)
        ...
        // 3. Construir URL y conectar (tu código actual, sin cambios)
    ...
    // También cierra el túnel al cerrar la conexión
    public boolean cerrar() {
        tunel.cerrarTunel(); // ← añadir
        if (conn == null) {
            return true;
        }
        try {
